...
- **`dev.ts`**: Enhanced the orchestrator script to automatically detect and launch the highest locally cached version of the Bifrost binary directly (copying it to the executable format if needed). This bypasses the `@maximhq/bifrost` version check, allowing the server to start successfully when offline.

## [1.4.11] - 2026-07-06

### Fixed
- **`src/components/DashboardTab.tsx`**: Wrapped the lightbox modal rendering inside a React Portal (`createPortal`) targeting `document.body` to resolve absolute/fixed positioning constraints caused by container transform styles (glassmorphism overlay).
